...
export default class WiSwipeAction extends WiComponent {
    ...
    render() {
        ...
        return <View id={`swipeAction-${componentId}`} className={rootClass} onTouchMove={this.handleTouchMove} onTouchEnd={this.handleTouchEnd} onTouchStart={this.handleTouchStart}>
            <View className={classNames('wi-swipe-action__content', {
            ...
            {Array.isArray(options) && options.length > 0 ? <WiSwipeActionOptions options={options} componentId={componentId} onQueryedDom={this.handleDomInfo}>
                {options.map((item, key) => <View key={`${item.text}-${key}`} style={item.style} onClick={this.handleClick.bind(this, item, key)} className={classNames('wi-swipe-action__option', item.className)}>
                    <Text className="option__text">{item.text}</Text>
                </View>)}
            </WiSwipeActionOptions> : null}
        </View>;
    }
}
