...
export default class WiButton extends WiComponent {
  ...
  renderBtn() {
    ...
    if (!disabled) {
      if (isWEB) {
        content = <Button className="wi-button__wxbutton" lang={lang} type={formType === 'submit' || formType === 'reset' ? formType : 'button'}>
                </Button>;
      } else {
        content = <Button className="wi-button__wxbutton" openType={openType} onGetUserInfo={this.onGetUserInfo.bind(this)} onGetPhoneNumber={this.onGetPhoneNumber.bind(this)} lang={lang}>
                </Button>;
      }
    }
