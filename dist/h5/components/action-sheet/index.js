...
export default class WiActionSheet extends WiComponent {
  ...
  renderTitle() {
    ...
    return <View className={rootClass}>{title}</View>;
  }

  renderFloor() {
    const { cancelText, className } = this.props;
    const rootClass = classNames('wi-action-sheet__footer', className);
    return <View onClick={this.close} className={rootClass}>{cancelText}</View>;
  }
  ...
  render() {
    ...
    const rootClass = classNames('wi-action-sheet', {
