 ...
 */
import Taro, { chooseImage as _chooseImage } from "@tarojs/taro-h5";
...
export default class WiImagePicker extends WiComponent {
  ...
  render() {
    ...
    return <View className={rootCls} style={customStyle}>
            {matrix.map((row, i) => <View className="wi-image-picker__flex-box" key={i + 1}>
                    {row.map((item, j) => item.url ? <View className="wi-image-picker__flex-item" key={i * length + j}>
                                <View className="wi-image-picker__item">
                                    <View className="wi-image-picker__remove-btn" onClick={this.handleRemoveImg.bind(this, i * length + j)} />
                                    <Image className="wi-image-picker__preview-img" mode={mode} src={item.url} onClick={this.handleImageClick.bind(this, i * length + j)} />
                                </View>
                            </View> : <View className="wi-image-picker__flex-item" key={i * length + j}>
                                {item.type === 'btn' && <View className="wi-image-picker__item wi-image-picker__choose-btn" onClick={this.chooseFile}>
